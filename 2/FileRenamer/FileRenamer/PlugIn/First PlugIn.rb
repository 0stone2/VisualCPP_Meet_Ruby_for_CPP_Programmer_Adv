...
def RenameAll()
	DbgString("[First PlugIn]Enter RenameAll\n")

	nSize = $leftPane.getSize()

	for nIndex in 0..(nSize - 1)
		file = $leftPane.getFile(nIndex) 
		DbgString(file + "\n")
		$rightPane.addFile(Rename(file), true)
	end 

	DbgString("[First PlugIn]Leave RenameAll\n")
end