...
static VALUE FileList_Initialize(VALUE self)
{
	OutputDebugString("Enter FileList_Initialize\n");

	rb_iv_set(self, "@files", rb_ary_new());

	return self;
}

static VALUE FileList_AddFile(VALUE self, VALUE file, VALUE UIUpdate )
{
	OutputDebugString("Enter FileList_AddFile\n");

	char *szFile = rb_string_value_cstr(&file);

	VALUE files = rb_iv_get(self, "@files");

	rb_ary_push(files, file);

	if (UIUpdate == RUBY_Qtrue)
	{
		CFileRenamerDlg *dlg = (CFileRenamerDlg *)AfxGetMainWnd();
		dlg->addFileToRightPane(szFile);
	}

	return Qnil;
}

static VALUE FileList_GetSize(VALUE self)
{
	OutputDebugString("Enter FileList_GetSize\n");

	VALUE files = rb_iv_get(self, "@files");

	int nSize = RARRAY_LEN(files);

	return rb_int2inum(nSize);
}

static VALUE FileList_GetFile(VALUE self, VALUE index)
{
	OutputDebugString("Enter FileList_GetFile\n");

	VALUE files = rb_iv_get(self, "@files");
	int nSize = RARRAY_LEN(files);
	int nIndex = rb_num2int(index);

	if(0 == nSize)
		return Qnil;
	else
		return rb_ary_entry(files, nIndex);
}
...
void CFileRenamerDlg::OnBnClickedRenameButton()
{
	...
	__try {
		...
		/////////////////////////////////////////////////////////////////////////////
		//VALUE	rb_cObject;
		VALUE cSFileList = rb_define_class("FileList", rb_cObject);

		rb_define_method(cSFileList, "initialize", (VALUE(*)(ANYARGS))FileList_Initialize, 0);
		rb_define_method(cSFileList, "addFile", (VALUE(*)(ANYARGS))FileList_AddFile, 2);
		rb_define_method(cSFileList, "getSize", (VALUE(*)(ANYARGS))FileList_GetSize, 0);
		rb_define_method(cSFileList, "getFile", (VALUE(*)(ANYARGS))FileList_GetFile, 1);
		...
		VALUE leftPane = rb_eval_string("$leftPane = FileList.new()");
		VALUE rightPane = rb_eval_string("$rightPane = FileList.new()");

		AddFile = rb_intern("addFile");
		for (nIndex = 0; nIndex < m_nNumberOfFiles; nIndex++)
		{
			ScriptFile = rb_str_new_cstr(m_FileList[nIndex].szSrcFile);
			RetString = rb_funcall(leftPane, AddFile, 2, ScriptFile, Qfalse);
		}

		Rename = rb_intern("RenameAll");
		RetString = rb_funcall(0, Rename, 0);
	}
	__finally {
...
BOOL CFileRenamerDlg::GetPlugInInfo(LPCSTR szPlugIn, PPLUGIN pPlugIn)
{
	...
	__try {
		...
		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/site_ruby/2.3.0/"));
		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/site_ruby/2.3.0/i386-msvcrt"));

		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/vendor_ruby"));
		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/vendor_ruby/2.3.0"));
		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/vendor_ruby/2.3.0/i386-msvcrt"));

		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/2.3.0"));
		rb_ary_push(load_path, rb_str_new_cstr("C:/Script/Ruby/lib/ruby/2.3.0/i386-mingw32"));
